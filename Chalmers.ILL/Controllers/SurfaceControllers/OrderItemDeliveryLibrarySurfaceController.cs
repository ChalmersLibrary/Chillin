...
namespace Chalmers.ILL.Controllers.SurfaceControllers
{
    ...
    public class OrderItemDeliveryLibrarySurfaceController : SurfaceController
    {
        ...
        /// <param name="deliveryInformation">Delivery Library Information</param>
        /// <returns>MVC ActionResult with JSON</returns>
        [HttpGet]
        public ActionResult SetOrderItemDeliveryReceived(int orderNodeId, string bookId, DateTime dueDate, string deliveryInformation)
        {
            var json = new ResultResponse();
            ...
            {
                // Use internal method to set status property and log the result
                _orderItemManager.SetOrderItemDeliveryReceivedInternal(orderNodeId, bookId, dueDate, deliveryInformation);

                // Construct JSON response for client (ie jQuery/getJSON)
                json.Success = true;
                json.Message = "Changed delivery item information to bookid:" + bookId + " due date:+" + dueDate + " delivery information:" + deliveryInformation;
            }
            catch (Exception e)
            {
                json.Success = false;
